...
/* Veränderungen zur vorangegangenen Version:
   - Auswahl der zu löschenden Elemente über Checkboxen
*/
...
if(count($_POST)>0) {
	...
	if(isset($_POST["btnAuswahl"])) {
		switch($_POST["selAuswahl"]) {
			case 1:
				//löschen
				for($i=0; $i<count($_POST["auswahl"]); $i++) {
					if(is_dir($_POST["auswahl"][$i])) {
						$ok = loescheVZ($_POST["auswahl"][$i]);
					}
					else {
						$ok = unlink($_POST["auswahl"][$i]);
					}
				}
				break;
		}
			...
			if($d!="." && $d!="..") {
				switch(true) {
					case is_dir($root.$d):
						$r.= '
							<tr>
								<td><input type="checkbox" name="auswahl[]" value="' .$root.$d. '/"></td>
	...
	<body>
		...
		<form id="DM" method="post" enctype="multipart/form-data">
			...
			<fieldset id="Auswahl">
				...
				<select name="selAuswahl">
					<option value="0">Bitte wählen:</option>
					<option value="1">Löschen</option>
				</select>
				<input type="submit" value="ausführen" name="btnAuswahl">
			</fieldset>
			<fieldset id="VZNeu">
